...
lcanv.addEventListener("click", function(e) {
	...
	if (clickPaths.length === 0) {
		// no paths exist to click
		return;
	}
...
gcviewContainer.style.width = (window.innerWidth-40)/2 + 'px';
gcviewContainer.style.height = (window.innerHeight-40) + 'px';
...
for (var i=0; i<ex.length; i++) {
	ex[i].addEventListener('click', function(e) {
		...
		alert('wait for loading: ' + file);

		r.addEventListener('progress', function(e) {
			// show progress of example loading
			console.log(e);
		});
